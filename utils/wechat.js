...
function CallWechat(appId, extend, sign, shopId, ssid) {
    var timestamp = Date.parse(new Date());
    var sign = md5(appId + extend + timestamp + shopId + "http://wifi.weixin.qq.com/assistant/wifigw/auth.xhtml?httpCode=200" + "aa:aa:aa:aa:aa:aa" + ssid + "532538acf2d5acf7cd671c768df0c6ac");
    Wechat_GotoRedirect(appId, extend, timestamp, sign, shopId, 'http://wifi.weixin.qq.com/assistant/wifigw/auth.xhtml?httpCode=200', 'aa:aa:aa:aa:aa:aa', ssid);
}
